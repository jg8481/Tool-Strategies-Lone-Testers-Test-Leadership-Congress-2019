...
Go to a YouTube page in an iOS mobile browser while Wireshark is capturing packets on WiFi, then save it to a packet capture file.
...
Go to a YouTube page in an iOS mobile browser while Wireshark is capturing packets on WiFi, then check the Loopback Interface.
    [Tags]    Mobile_Safari    Wireshark    Wireshark_IOS
    [Setup]    Navigate To Website Using Safari Then Run Wireshark
    Run Wireshark Live Packet Capture For Loopback Interface Lo0 And Check The Output
    [Teardown]    Close Applications And Terminate Wireshark Processes

Go to a YouTube page in an iOS mobile browser while Wireshark is capturing packets, then run a Lua script to analyze the results.
    [Tags]    Mobile_Safari    Wireshark    Wireshark_IOS
    [Setup]    Navigate To Website Using Safari Then Run Wireshark
